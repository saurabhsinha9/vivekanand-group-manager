...
// src/pages/AlbumManager.jsx
import { useEffect, useState } from 'react';
import { useParams } from 'react-router-dom';
import api from '../api/axios';
import {
  Container, Paper, Typography, Grid, TextField, Button, Card, CardMedia, CardContent,
...
function SortableItem({ item, onEdit, onDelete }) {
  ...
  return (
    <Grid item xs={12} sm={6} md={4} ref={setNodeRef} style={style}>
      <Card>
        {/* Use poster endpoint for thumbnails (works for images & videos) */}
        <CardMedia
          component="img"
          height="180"
          image={`${import.meta.env.VITE_API_URL || 'http://localhost:8080/api'}/public/uploads/${item.uploadId}/poster`}
          alt={item.title || item?.originalFilename || 'media'}
          loading="lazy"
        />
        <CardContent>
...
export default function AlbumManager() {
  ...
  const updateItem = async (id, payload) => {
    await api.put(`/gallery/admin/items/${id}`, payload);
    load();
  };

  const deleteItem = async (id) => {
    await api.delete(`/gallery/admin/items/${id}`);
    load();
  };
      ...
      <Paper sx={{ p: 2, mt: 2 }}>
        ...
        <Grid container spacing={2}>
          ...
          <Grid item xs={12} sm={6}>
            ...
            {/* Preview selected upload: image or video with poster */}
            {selectedUpload?.contentType?.startsWith('image/') ? (
              <Card sx={{ mt: 1 }}>
                <CardMedia
                  component="img"
                  height="140"
                  image={`${import.meta.env.VITE_API_URL || 'http://localhost:8080/api'}/public/uploads/${selectedUpload.id}`}
                  alt={selectedUpload.originalFilename ?? 'preview'}
                  loading="lazy"
                />
              </Card>
              ...
              <Card sx={{ mt: 1 }}>
                <video
                  src={`${import.meta.env.VITE_API_URL || 'http://localhost:8080/api'}/public/uploads/${selectedUpload.id}`}
                  poster={`${import.meta.env.VITE_API_URL || 'http://localhost:8080/api'}/public/uploads/${selectedUpload.id}/poster`}
                  height="140"
                  controls
                  muted
                  playsInline
